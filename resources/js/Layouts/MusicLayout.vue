          ...
          <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div class="hidden sm:ml-6 sm:block">
              <div class="flex space-x-4">
                ...
                <Link v-if="$page.props.auth.user" :href="route('playlist.index')"
                ...
                <Link v-if="$page.props.auth.user" :href="route('apikey.index')"
                  class="rounded-md px-3 py-2 text-sm font-medium"
                  :class="[route().current().includes('apikey') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white']">
                Clé Api
                </Link>
              </div>
            </div>
